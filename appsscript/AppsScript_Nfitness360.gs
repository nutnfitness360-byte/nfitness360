...
var ISAK_NAME = "ISAK";
var RECOMENDACIONES_NAME = "Recomendaciones";
...
function doPost(e) {
  try {
    ...
    if (action === "saveRecomendaciones") {
      var fnameR = (body.filename || ("Recomendaciones_" + patient + ".pdf")).toString();
      if (fnameR.slice(-4).toLowerCase() !== ".pdf") fnameR += ".pdf";

      var blobR;
      if (body.html) {
        blobR = Utilities.newBlob(body.html, "text/html", fnameR).getAs("application/pdf");
        blobR.setName(fnameR);
      } else if (body.pdfBase64) {
        var b64r = body.pdfBase64;
        var cir = b64r.indexOf("base64,");
        if (cir > -1) b64r = b64r.substring(cir + 7);
        blobR = Utilities.newBlob(Utilities.base64Decode(b64r), "application/pdf", fnameR);
      } else {
        throw new Error("Falta 'html' o 'pdfBase64'.");
      }
      ...
      return json_({ ok: true, action: action, patient: patient,
                     ...
                     filename: fnameR, recomendacionesFolder: recFolder.getUrl() });
    }
    ...
    // Acciones definidas en "Handlers faltantes.gs"
    if (action === "kcalHabitualIA") {
      return kcalHabitualIA_(body);
    }

    if (action === "leerInBody") {
      return leerInBody_(body);
    }

    if (action === "ajustarMenuIA") {
      return ajustarMenuIA_(body);
    }
 ...
 *  La llave secreta va SOLO en las Propiedades del script (STRIPE_SECRET_KEY),
 *  nunca en el código. Usa 'sk_test_...' para pruebas y 'sk_live_...' en producción.
 * ═══════════════════════════════════════════════════════════════════ */
function stripeKey_() {
  return PropertiesService.getScriptProperties().getProperty('STRIPE_SECRET_KEY');
}

/*  Acción: { action:"crearCheckoutStripe", montoCentavos, descripcion, correo, citaId, successUrl, cancelUrl }
 *  Respuesta: { ok:true, url:"https://checkout.stripe.com/..." }  ó  { ok:false, error }
 */
function crearCheckoutStripe_(body) {
  var key = stripeKey_();
  if (!key) return json_({ ok: false, error: "Falta STRIPE_SECRET_KEY en las Propiedades del script." });

  var monto = Math.round(Number(body.montoCentavos) || 0);
  if (!(monto > 0)) return json_({ ok: false, error: "Monto inválido." });

  var descripcion = (body.descripcion || "Consulta").toString();
  var correo = (body.correo || "").toString().trim();
  var citaId = (body.citaId || "").toString();
  var successUrl = (body.successUrl || "").toString();
  var cancelUrl = (body.cancelUrl || "").toString();

  // Stripe recibe form-urlencoded con notación de corchetes.
  var params = {
    "mode": "payment",
    "success_url": successUrl,
    "cancel_url": cancelUrl,
    "client_reference_id": citaId,
    "metadata[citaId]": citaId,
    "line_items[0][quantity]": "1",
    "line_items[0][price_data][currency]": "mxn",
    "line_items[0][price_data][unit_amount]": String(monto),
    "line_items[0][price_data][product_data][name]": descripcion
  };
  if (correo && correo.indexOf("@") > -1) params["customer_email"] = correo;

  var payload = Object.keys(params).map(function (k) {
  ...
  if (code >= 200 && code < 300 && data && data.url) {
    return json_({ ok: true, url: data.url, id: data.id });
  }
  var msg = (data && data.error && data.error.message) ? data.error.message : ("Stripe respondió " + code);
  return json_({ ok: false, error: msg });
}

/*  Acción: { action:"verificarPagoStripe", sessionId }
 *  Respuesta: { ok:true, pagado:true|false }  ó  { ok:false, error }
 */
function verificarPagoStripe_(body) {
  var key = stripeKey_();
  if (!key) return json_({ ok: false, error: "Falta STRIPE_SECRET_KEY en las Propiedades del script." });

  var sessionId = (body.sessionId || "").toString();
  if (!sessionId) return json_({ ok: false, error: "Falta sessionId." });
  ...
  if (code >= 200 && code < 300 && data) {
    return json_({ ok: true, pagado: (data.payment_status === "paid"), estado: data.payment_status || "" });
  }
  var msg = (data && data.error && data.error.message) ? data.error.message : ("Stripe respondió " + code);
  return json_({ ok: false, error: msg });
}

// ---- Comparte un archivo para que el paciente lo vea SIN solicitar acceso ----
// Se pone como "cualquiera con el enlace puede ver" (solo lectura): el paciente
// abre su archivo aunque no tenga cuenta de Google o esté en otra cuenta, y no se
// detiene la operación pidiendo acceso. El enlace solo se le envía a él.
  ...
  try {
    var interno =
      ...
      "<ul>" +
        ...
        (objetivo ? "<li><strong>Objetivo:</strong> " + escapar_(objetivo) + "</li>" : "") +
        (notas ? "<li><strong>Notas:</strong> " + escapar_(notas) + "</li>" : "") +
        (online && meetLink ? "<li><strong>Meet:</strong> " + escapar_(meetLink) + "</li>" : "") +
      "</ul></div>";
    MailApp.sendEmail({ to: CORREO_CONT, subject: "Nueva cita: " + nombre + " · " + fecha + " " + hora, htmlBody: interno, name: NEGOCIO });
  } catch (eInterno) { /* el aviso interno es secundario; no debe romper el agendado */ }
...
function recetarioMuestra_(cat, n) {
  if (!cat || typeof RECETARIO === 'undefined' || !RECETARIO[cat]) return [];
  var arr = RECETARIO[cat].slice();
...
function generarMenusIA_(body) {
  ...
  var sys = "Eres asistente de una nutrióloga mexicana. Propones platillos concretos para un plan basado en el " +
    ...
    "Cuentas con un RECETARIO de platillos reales ya validados por la nutrióloga: para cada tiempo recibes " +
    "'candidatos_recetario' (platillos apropiados para ese tiempo, con sus componentes por 1 equivalente y su preparación). " +
  ...
  var datos = {
    ...
    consideraciones_generales: (body.consideraciones || "").toString(),
 ...
 *  Correo automático a quien lleva X días sin un plan nuevo (X lo define
 *  la nutrióloga desde Configuración). Reutiliza la bitácora de seguimiento:
 *  columna 6 = reactivación enviada, columna 7 = fecha de envío.
 *  Anti-spam: un correo por racha de inactividad; al llegar un plan nuevo
 *  se agrega una fila nueva (sin marca), así el paciente vuelve a ser
 *  elegible si se inactiva otra vez.
 *  Propiedades del script: REACT_ACTIVA ('1'/''), REACT_DIAS (número).
 * ═══════════════════════════════════════════════════════════════════ */

// El app guarda aquí la configuración (encendido + días).
function setReactivacion_(body) {
  var props = PropertiesService.getScriptProperties();
  props.setProperty('REACT_ACTIVA', body.activo ? '1' : '');
  var dias = parseInt(body.dias, 10);
  if (!isNaN(dias) && dias > 0) props.setProperty('REACT_DIAS', String(dias));
  return json_({ ok: true, activo: !!body.activo, dias: props.getProperty('REACT_DIAS') || '' });
}

// Disparador diario: manda reactivación a inactivos (una vez por racha).
function enviarReactivaciones_() {
  var props = PropertiesService.getScriptProperties();
  if (props.getProperty('REACT_ACTIVA') !== '1') return 0; // apagado
  var dias = parseInt(props.getProperty('REACT_DIAS') || '45', 10);
  ...
  // Última fila (plan más reciente) por correo.
  var ultima = {};
  for (var i = 1; i < data.length; i++) {
    var correo = String(data[i][0] || '').trim().toLowerCase();
    if (!correo) continue;
    var fechaPlan = String(data[i][2] || '');
    if (!fechaPlan) continue;
    if (!ultima[correo] || fechaPlan > ultima[correo].fechaPlan) {
      ultima[correo] = { row: i + 1, fechaPlan: fechaPlan, nombre: String(data[i][1] || 'Paciente'), react: String(data[i][5] || '') };
    }
  }
  ...
  for (var c in ultima) {
    var u = ultima[c];
    if (u.react === '1') continue; // ya se le envió en esta racha
    var d = new Date(u.fechaPlan + 'T00:00:00');
    if (isNaN(d)) continue;
    var transcurridos = Math.floor((hoy.getTime() - d.getTime()) / 86400000);
    if (transcurridos < dias) continue;
    try {
      MailApp.sendEmail({
        to: c,
        subject: "¿Retomamos tu seguimiento? | " + NEGOCIO,
        htmlBody: correoReactivacion_(u.nombre),
        name: NUTRIOLOGA + " · " + NEGOCIO
      });
      sh.getRange(u.row, 6).setValue('1');
      sh.getRange(u.row, 7).setValue(Utilities.formatDate(hoy, TZ, 'yyyy-MM-dd'));
      enviados.push(u.nombre + " <" + c + ">");
    } catch (e) { /* sigue con los demás aunque uno falle */ }
  }

  // Resumen a la nutrióloga (solo si hubo envíos).
  if (enviados.length) {
    try {
      MailApp.sendEmail({
        to: CORREO_CONT,
        subject: "Reactivación enviada a " + enviados.length + " paciente(s) · " + NEGOCIO,
        htmlBody: "<div style='font-family:Montserrat,Arial,sans-serif;color:" + C_INK + ";font-size:13px;line-height:1.7;'>" +
          "<p>Hoy se envió el correo de reactivación (inactividad de " + dias + " días o más) a:</p><ul>" +
          enviados.map(function (x) { return "<li>" + escapar_(x) + "</li>"; }).join("") + "</ul></div>",
        name: NEGOCIO
      });
    } catch (e) {}
  }
  Logger.log("Reactivaciones enviadas: " + enviados.length);
  return enviados.length;
}

// Instala el disparador diario (correr UNA vez desde el editor).
function instalarTriggerReactivaciones() {
  var triggers = ScriptApp.getProjectTriggers();
  for (var i = 0; i < triggers.length; i++) {
    if (triggers[i].getHandlerFunction() === "enviarReactivaciones_") ScriptApp.deleteTrigger(triggers[i]);
  }
  ScriptApp.newTrigger("enviarReactivaciones_").timeBased().everyDays(1).atHour(8).inTimezone(TZ).create();
  return "Trigger de reactivación instalado (diario ~8:00 a.m.).";
}

// Correo de reactivación (voz de Natalia, con botón a Calendly).
function correoReactivacion_(nombre) {
  var CALENDLY = "https://calendly.com/nutnfitness360";
  var boton =
    "<div style='text-align:center;margin:22px 0 6px;'>" +
      "<a href='" + CALENDLY + "' style='display:inline-block;background:" + C_PINE + ";color:#fff;text-decoration:none;font-weight:700;font-size:14px;padding:13px 30px;border-radius:11px;'>Agendar mi seguimiento</a>" +
    "</div>";
  var cuerpo =
    "<p style='font-size:14px;'>Hola <strong>" + escapar_(nombre) + "</strong>:</p>" +
    "<p style='font-size:13px;line-height:1.6;'>Te escribo porque hace un tiempo que no nos vemos y quería saber cómo has estado. La vida se pone ocupada y a veces el seguimiento se pausa — no pasa nada, aquí sigo para cuando quieras retomar.</p>" +
    "<p style='font-size:13px;line-height:1.6;'>Lo bueno es que tu proceso no empieza de cero: lo que ya avanzaste sigue contando. Con una consulta de seguimiento podemos ajustar tu plan a cómo estás hoy y volver a encaminarte hacia tus objetivos.</p>" +
    boton +
    "<p style='margin:18px 0 0;font-size:13px;line-height:1.6;'>Y si tienes cualquier duda, respóndeme este correo con confianza. Me daría mucho gusto acompañarte de nuevo.</p>";
  return wrapEmail_(cuerpo);
}

// PRUEBA: envíate a ti misma el correo de reactivación.
function PRUEBA_reactivacion() {
  MailApp.sendEmail({ to: CORREO_CONT, subject: "PRUEBA Reactivación | " + NEGOCIO, htmlBody: correoReactivacion_("Paciente Prueba"), name: NUTRIOLOGA + " · " + NEGOCIO });
}
