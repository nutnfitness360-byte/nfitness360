...
const T = {
  bg: "var(--cream)",        // crema de marca (configurable)
  surface: "var(--card)",    // tarjetas (configurable)
  ink: "var(--ink)",         // texto oscuro cálido (configurable)
  inkSoft: "var(--stone)",   // texto secundario (configurable)
  line: "var(--border)",     // bordes (configurable)
  lineSoft: "var(--border)", // líneas suaves (configurable)
  pine: "var(--dark)",       // oscuro: botones, píldoras, encabezados (configurable)
  pineSoft: "#000000",       // negro fijo (texto fuerte)
  amber: "var(--gold)",      // dorado de marca / acento (configurable)
  mint: "var(--mint)",       // tinte crema, fondos suaves (configurable)
  danger: "var(--danger)",   // alertas / rojo (configurable)
  sage: "var(--sage)",       // verde de marca (configurable)
  black: "#000000",          // barra del logo (negro fijo)
};

// El PDF se rasteriza desde un HTML aislado: como el css usa var(--…), inyectamos
// aquí los colores configurados actuales para que el documento salga con esa paleta.
function pdfRootVars() {
  try {
    const cs = getComputedStyle(document.documentElement);
    const g = (k) => (cs.getPropertyValue(k) || '').trim();
    const keys = ['--cream', '--gold', '--sage', '--stone', '--dark', '--card', '--border', '--ink', '--mint', '--danger'];
    return ':root{' + keys.map(k => `${k}:${g(k)};`).join('') + '}';
  } catch (e) { return ''; }
}
...
td.momento { background: var(--mint); font-weight: 700; color: ${T.pine}; width: 130px; }
...
async function generarHistorialPDFBase64(data) {
  ...
  host.innerHTML = `<style>${MONT_CSS}${pdfRootVars()}${css}</style><div id="nf-hdr" class="pagehdr">${header}</div><div id="nf-cards">${content}</div>`;
        ...
        <div style={styles.titleRow}>
          <div>
            ...
            <div style={styles.idBlock}>
              <span style={styles.noPill}>{data.datos.pacienteNo || (CODE_PREFIX + "…")}</span>
      ...
      <main style={styles.main}>
        ...
        <Section reg={reg("datos")} sid="datos" title="Datos generales" n="1"
          ...
          <Grid>
            ...
            <Field label="Estatura / talla (cm)">
              <input style={styles.input} inputMode="decimal" value={data.datos.talla || ""}
                placeholder="Ej. 165"
                onChange={(e) => setField("datos", "talla", e.target.value.replace(/[^0-9.]/g, ""))} />
            </Field>
          ...
          <p style={styles.note}>
            La edad, el sexo y la talla capturados aquí, junto con el peso registrado en el expediente (InBody),
            ...
            {data.dietetica.kcalHabitual && (
              <div style={{ marginTop: 12, border: `1px solid ${T.line}`, borderRadius: 12, padding: "14px 16px", background: T.surface }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 12 }}>
                  ...
                  <div style={{ flex: "1 1 90px", background: "var(--card)", border: `1px solid ${T.line}`, borderRadius: 9, padding: "10px 12px" }}>
...
const styles = {
  ...
  noPill: { background: T.amber, color: "var(--dark)", fontWeight: 800, fontSize: 12.5, padding: "4px 11px", borderRadius: 7, letterSpacing: 0.6, display: "inline-block" },
  ...
  input: { width: "100%", border: `1px solid ${T.line}`, borderRadius: 9, padding: "10px 12px", fontSize: 14, color: T.ink, background: "var(--card)", boxSizing: "border-box", fontFamily: "inherit" },
  ...
  autoTag: { position: "absolute", right: 10, fontSize: 10.5, fontWeight: 700, color: T.pineSoft, background: "var(--card)", border: `1px solid ${T.line}`, borderRadius: 6, padding: "1px 6px", textTransform: "uppercase", letterSpacing: 0.5 },
  textarea: { width: "100%", border: `1px solid ${T.line}`, borderRadius: 9, padding: "10px 12px", fontSize: 14, color: T.ink, background: "var(--card)", boxSizing: "border-box", resize: "vertical", fontFamily: "inherit", lineHeight: 1.5 },
  ...
  footer: { margin: "10px 24px 0", background: "var(--card)", border: `1px solid ${T.line}`, borderRadius: 14, padding: "16px 20px", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "flex-end", gap: 12 },
  ...
  secondaryBtn: { background: "var(--card)", color: T.pine, border: `1px solid ${T.pine}`, padding: "11px 16px", borderRadius: 11, fontSize: 13.5, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" },
  primaryBtn: { background: T.amber, color: "var(--dark)", border: "none", padding: "11px 18px", borderRadius: 11, fontSize: 13.5, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" },
  ...
  modalBox: { background: "var(--card)", borderRadius: 16, padding: "26px 24px", maxWidth: 380, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.25)", textAlign: "center" },
  ...
  modalNo: { flex: 1, background: "var(--card)", color: T.ink, border: `1px solid ${T.line}`, padding: "11px 0", borderRadius: 10, fontSize: 14.5, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" },
  modalSi: { flex: 1, background: T.amber, color: "var(--dark)", border: "none", padding: "11px 0", borderRadius: 10, fontSize: 14.5, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" },
};
...
input:disabled { background: var(--mint); color: var(--stone); }
